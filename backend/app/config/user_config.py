...
class UserConfig:
    #Loading and managing the user configurations from .env file
        ...
        try:
            ...
            for user in users:
                if not all(field in user for field in required_fields):
                    print(f"Warning: User {user.get('username', 'unknown')} is missing required fields")
                    continue
                
                # Validating role
                if user['role'] not in ['admin', 'hr']:
                    print(f"Warning: User {user['username']} has invalid role: {user['role']}")
    ...
    def get_session_config() -> Dict:
        #Getting session configuration from environment
        return {
    ...
    def validate_env_config():
        ...
        # Checking if default users are configured
        users = UserConfig.get_default_users()
        if not users:
            print("Warning: No default users configured in DEFAULT_USERS")
            return False
